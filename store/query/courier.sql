...
SELECT DISTINCT ON (p.relevance) c.id, p.* FROM couriers c INNER JOIN products p ON ST_DWithin(c.location, sqlc.arg(point)::geography, 1000) ORDER BY p.relevance DESC;
