...
func (u *UserRepository) Init(q *sqlc.Queries) {
...
func (u *UserRepository) FindOrCreateCourier(userID uuid.UUID) (*model.Courier, error) {
	...
	if err == nil && courier == nil {
		...
		if newErr != nil {
			...
			}).WithError(newErr).Errorf("find/create courier")
			return nil, newErr
		}
...
func (u *UserRepository) getCourierByUserID(userID uuid.UUID) (*model.Courier, error) {
	...
	if err == sql.ErrNoRows {
		...
		u.log.WithFields(logrus.Fields{
			"user_id": userID,
		}).WithError(err).Errorf("get courier by id")
		return nil, err
	}
...
func (u *UserRepository) createUser(user SigninInput) (*model.User, error) {
	...
	if newUserErr != nil {
		u.log.WithFields(logrus.Fields{
			"user": user,
		}).WithError(newUserErr).Errorf("create new user")
...
func (u *UserRepository) getUserByPhone(phone string) (*model.User, error) {
	...
	if getErr == sql.ErrNoRows {
		...
		u.log.WithFields(logrus.Fields{
			"phone": phone,
		}).WithError(getErr).Errorf("get user by phone")
		return nil, getErr
	}
...
func (u *UserRepository) findUserByID(id uuid.UUID) (*model.User, error) {
	...
	if getErr == sql.ErrNoRows {
		u.log.WithFields(logrus.Fields{
			"user_id": id,
		}).WithError(userNotFound).Errorf(userNotFound.Error())
		return nil, userNotFound
	} else if getErr != nil {
		u.log.WithFields(logrus.Fields{
			"user_id": id,
		}).WithError(getErr).Errorf("get user by id")
		return nil, getErr
	}
...
func (u *UserRepository) OnboardUser(user SigninInput) (*model.User, error) {
	...
	if onboardErr != nil {
		u.log.WithFields(logrus.Fields{
			"args": updateArgs,
		}).WithError(onboardErr).Errorf("update user name")
	...
	if _, err := u.store.SetOnboardingStatus(context.Background(), statusArgs); err != nil {
		u.log.WithFields(logrus.Fields{
			"phone": user.Phone,
		}).WithError(err).Errorf("set user onboarding status")
		return nil, err
	}
...
func (u *UserRepository) getSession(userID uuid.UUID) (*model.Session, error) {
	...
	if sessErr == sql.ErrNoRows {
		...
		u.log.WithFields(logrus.Fields{
			"user_id": userID,
		}).WithError(sessErr).Errorf("get user session")
		return nil, sessErr
	}
...
func (u *UserRepository) createNewSession(
	...
	if newSessErr != nil {
		...
		}).WithError(newSessErr).Errorf("create new session")
		return nil, newSessErr
	}
...
func (u *UserRepository) getCourierStatus(userID uuid.UUID) (model.CourierStatus, error) {
	...
	if err == sql.ErrNoRows {
		...
		u.log.WithFields(logrus.Fields{
			"courier_user_id": userID,
		}).WithError(err).Errorf("get courier status")
...
func (u *UserRepository) isCourier(userID uuid.UUID) (bool, error) {
	...
	if err == sql.ErrNoRows {
		...
		u.log.WithFields(logrus.Fields{
			"courier_user_id": userID,
		}).WithError(err).Errorf("is courier check")
		return false, err
	}
